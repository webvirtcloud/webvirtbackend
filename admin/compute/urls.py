...
urlpatterns = [
    ...
    re_path(
        "(?P<pk>\d+)/storages/(?P<pool>[\w\d\-]+)/delete/?$", 
        AdminComputeStorageDeleteView.as_view(), name="admin_compute_storage_delete"
    ),
    re_path(
    ...
    re_path(
        ...
        AdminComputeNetworkDeleteView.as_view(), name="admin_compute_network_delete"
    ),

    re_path("(?P<pk>\d+)/secrets/?$", AdminComputeSecretsView.as_view(), name="admin_compute_secrets"),
    re_path(
        "(?P<pk>\d+)/secrets/create/?$",
        AdminComputeSecretCreateView.as_view(), name="admin_compute_secret_create"
    ),
    re_path(
        "(?P<pk>\d+)/secret/(?P<uuid>[\w\d\-]+)/value/?$",
        AdminComputeSecretValueView.as_view(), name="admin_compute_secret_value"
    ),
    re_path(
        "(?P<pk>\d+)/secret/(?P<uuid>[\w\d\-]+)/delete/?$",
        AdminComputeSecretDeleteView.as_view(), name="admin_compute_secret_delete"
    ),
